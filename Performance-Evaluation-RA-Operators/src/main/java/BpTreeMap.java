       ...
       implements Serializable, Cloneable, SortedMap <K, V>
{
    ...
    /** An internal node to hold keys and references/pointers of left child after splitting
     */
    ...
    public Set <Entry <K, V>> entrySet ()
    {
        ...
        Node currentNode = firstLeaf;                                                // Assigning firstLeaf Node as currentNode
        SortedMap<K,V> keyValues= new TreeMap<K, V>();

        // Checking whether Leaf Node is empty
        while(currentNode.nKeys>0) {
            for(int i=0;i<currentNode.nKeys;i++){                              // Iterating each leaf node
                keyValues.put(currentNode.key[i], (V)currentNode.ref[i]); // Inserting required K,V in Map


            }

            if(currentNode.ref[currentNode.nKeys]!=null) {
                currentNode= (Node) currentNode.ref[currentNode.nKeys];       // Fetching new leaf node after iterating current leaf node
            }else{
                break;                                                        // Breaking while loop if we reach last leaf
            }
        }

        for (SortedMap.Entry<K,V> entry : keyValues.entrySet())                // for loop to entrySet to SET
        {
            enSet.add(entry);
        }
     ...
     */
    public V get (Object key) { return find ((K) key, root);} // get
    ...
    public K lastKey ()
    {
        ...
        Node lastNode;
        if(root.isLeaf && root.nKeys==0){                            // Checking whether root is leaf and non-empty
            return null;
        }
        else if(root.isLeaf && root.ref[root.nKeys]==null){         // Checking whether root is leaf
            return root.key[root.nKeys-1];
        }
        else{                                                      // Else block for non-leaf nodes

            lastNode= (Node)root.ref[root.nKeys];

            while( !lastNode.isLeaf ) {                            // While block to reach last leaf Node
                lastNode = (Node) lastNode.ref[lastNode.nKeys];
            }

            return lastNode.key[lastNode.nKeys-1];                // return lastKey from last leaf node
        }
    ...
    public SortedMap <K,V> headMap (K toKey)
    {
        ...
        Node currentNode = firstLeaf;                                                // Assigning firstLeaf Node as currentNode
        SortedMap<K,V> keyValues= new BpTreeMap(classK,classV);
        this.entrySet();

        if(currentNode.nKeys>0){                                                    // Checking whether Leaf Node is empty
            while((currentNode.key[0].compareTo(toKey)<0) )
            {
                for(int i=0;i<currentNode.nKeys;i++){                              // Iterating each leaf node
                    if( currentNode.key[i].compareTo(toKey)<0 ){                   // Checking key < toKey
                        keyValues.put(currentNode.key[i], (V) currentNode.ref[i]); // Inserting required K,V in Map
    ...
    public SortedMap <K,V> tailMap (K fromKey)
    {
        ...
        SortedMap<K,V> keyValues= new BpTreeMap(classK,classV);
        int pointerReference;

        /*********************************************************************************
         * Loop to reach leaf node which contains "fromKey"
         *
         */

        while (!currentNode.isLeaf){
            pointerReference=currentNode.find(fromKey);                               // Getting the pointer reference from each intermediate node to with find method
            currentNode=(Node) currentNode.ref[pointerReference];
        }

        /*********************************************************************************
         * Loop to get the the portion of the B+Tree where fromKey <= key
         *
         */
        while (currentNode.nKeys>0){

            for(int i=0;i<currentNode.nKeys;i++){                                 // Iterating each node
                if((currentNode.key[i].compareTo(fromKey)>=0)){                  // Checking whether fromKey <= currentkey
                    keyValues.put(currentNode.key[i], (V) currentNode.ref[i]);  // Inserting required K,V in Map
    ...
    public SortedMap <K,V> subMap (K fromKey, K toKey)
    {
        ...
        /*********************************************************************************
         * Loop to get the the portion of the B+Tree whose keys are between fromKey and toKey
         */

        while((currentNode.nKeys>0) && (currentNode.key[0].compareTo(toKey)<0)){

            for(int i=0;i<currentNode.nKeys;i++){                                                                    // Iterating each node
                if((currentNode.key[i].compareTo(fromKey)>=0) && (currentNode.key[i].compareTo(toKey)<0)){          // Checking  keys in the range [fromKey, toKey)
                    keyValues.put(currentNode.key[i], (V) currentNode.ref[i]);                                      // Inserting required K,V in Map
    ...
    {
        if (n == root) out.println ("BpTreeMap_insertWorks");
        out.println ("-------------------------------------------");
    ...
    private Node insert (K key, V ref, Node n)
    {
       ...
       // out.println ("=============================================================");
        ...
        if (n.isLeaf) {                                                      // handle leaf node level
            ...
            //  T O   B E   I M P L E M E N T E D
            if (hasSplit == true) {         // update only when child node is split
                hasSplit = false;
                Node ll = (Node) n.ref[i];
                if (n.nKeys < ORDER - 1) {  // space available in this node - can insert here
                    wedge(ll.key[ll.nKeys - 1], rt, n, n.find(ll.key[ll.nKeys - 1]), false);
                    if (!ll.isLeaf) {      // for internal node, delete the largest key in left child
//                        System.out.println("1 -- erasing key " + ll.key[ll.nKeys - 1] + ", ll.nKeys: " + ll.nKeys + ", ll: " + ll);
                        ll.key[ll.nKeys - 1] = null;
                        ll.ref[ll.nKeys] = null;
                        ll.nKeys--;
                    }
                }
                else {                      // space not avaiable - needs to split
//                    System.out.println("space not avaiable, needs splitting: Adding ll.key[ll.nKeys - 1]: " + ll.key[ll.nKeys - 1]);
                    rt = split(ll.key[ll.nKeys - 1], rt, n, false);
                    if (n == root && rt != null) {      // if root is split, update root
                        root = makeRoot(n, n.key[n.nKeys - 1], rt);
//                        System.out.println("2 -- erasing key in child " + n.key[n.nKeys - 1] + ", n.nKeys: " + n.nKeys + ", n: " + n);
                        n.key[n.nKeys - 1] = null;
                        n.ref[n.nKeys] = null;
                        n.nKeys--;
                        if (in != null && !in.isLeaf) { // for internal node, delete the largest key in left child
//                            System.out.println("3 -- erasing key internal key " + in.key[in.nKeys - 1] + ", in.nKeys: " + in.nKeys + ", in: " + in);
                            in.key[in.nKeys - 1] = null;
                            in.ref[in.nKeys] = null;
                            in.nKeys--;
                            in = null;
                        }
                    }
                    else if (rt != null) {              // if not root, notify parent that there's a split here
                        hasSplit = true;
                        if (in != null && !in.isLeaf) { // for internal node, delete the largest key in left child
//                            System.out.println("4 -- erasing key internal key " + in.key[in.nKeys - 1] + ", in.nKeys: " + in.nKeys + ", in: " + in);
                            in.key[in.nKeys - 1] = null;
                            in.ref[in.nKeys] = null;
                            in.nKeys--;
                            in = null;
    ...
    private Node makeRoot (Node ref0, K key0, Node ref1)
    {
       ...
       // System.out.println("In makeRoot: " + key0);
        return nr;
    } // makeRoot
    ...
    private boolean wedge (K key, Object ref, Node n, int i, boolean left)
    {
        if (i < n.nKeys && key.compareTo(n.key[i]) == 0) {
             out.println ("BpTreeMap_insertWorks.insert: attempt to insert duplicate key = " + key);
             return false;
        } // if
    ...
    public static void main (String [] args)
    {
        ...
        int p[] = new int[]{30,30,11,35,2,31,25,4,20,0,1,9,26,21,6,39,17,16,30,37};
        ...
        if (RANDOMLY) {
            ...
            int q[] = new int[]{100,200,300,400,500,600,700,800,900,850,950,825,875,840,860,830,820,818,810};
           // for(int i=0; i < q.length;i++)bpt.put(q[i],i*i);
//           for (int i = 0; i < totalKeys; ++i) {q[i] = rng.nextInt (2*totalKeys); System.out.print(q[i] + ", ");}
            for (int i = 0; i < p.length; i++) bpt.put (p[i], i * i);
        } else {
            for (int i = 1; i <= totalKeys; i += 2) bpt.put (i, i * i);
        } // if

        // check for this order: 9 31 2 10 39 0 36 14 16 25 and ORDER = 3
