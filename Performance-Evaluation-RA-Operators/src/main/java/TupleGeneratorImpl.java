        ...
        implements TupleGenerator
{
    ...
    {   int tuplelimit=Integer.MAX_VALUE;
                ...
                for (int j = 0; j < tuples[i]; j++) {
                    for (int k = 0; k < attribute.length; k++) {
                        if (pKeys.contains (attribute[k])) {  // key requires uniqueness
                            ...
                            switch (domain[k]) {
                                case "Integer":
                                    for (iVal = rand.nextInt (tuplelimit); pKeyValues.contains (iVal);
                                         iVal = rand.nextInt (tuplelimit));
                                    subResult[j][k] = iVal;
                                    pKeyValues.add (iVal);
                                    break;
                                case "String":
                                    for (sVal = attribute[k] + rand.nextInt (tuplelimit); pKeyValues.contains(sVal);
                                         sVal = attribute[k] + rand.nextInt (tuplelimit));
                                    subResult[j][k] = sVal;
                                    pKeyValues.add (sVal);
                                    break;
                                case "Double":
                                    for (dVal = rand.nextInt (tuplelimit) * rand.nextDouble (); pKeyValues.contains( dVal);
                                         dVal = rand.nextInt (tuplelimit) * rand.nextDouble ());
                                    subResult[j][k] = dVal;
                                    pKeyValues.add (dVal);
                                    break;
                                default:
                            ...
                            switch (domain[k]) {
                                case "Integer":
                                    subResult[j][k] = rand.nextInt(tuplelimit); break;
                                case "String":
                                    subResult[j][k] = attribute [k] + rand.nextInt (tuplelimit); break;
                                case "Double":
                            ...
                            if (pKeys.contains (attribute[k])) {
                                ...
                                switch (domain[k]) {
                                    ...
                                    case "String":
                                        subResult[j][k] = attribute [k] + rand.nextInt (tuplelimit); break;
                                    case "Double":
                                        subResult[j][k] = rand.nextInt (tuplelimit) * rand.nextDouble (); break;
                                    default:
                                        throw new IllegalArgumentException("Invalid type in switch: " + domain[k]);
                                } // if
