        ...
        implements Serializable
{
    ...
    public Table select (KeyType keyVal)
    {
        ...
        */
         ...
         if(mType==MapType.NO_MAP){
             for (Comparable[] record:tuples) {
                
                 if(new KeyType(record[match(key)[0]]).compareTo(keyVal)==0){
                     rows.add(record);
    ...
    public Table select (KeyType keyVal1, KeyType keyVal2)
    {
       // out.println ("RA> " + name + ".select between (" + keyVal1 + ") and " + keyVal2);
        ...
        if(mType==MapType.NO_MAP){
            //System.out.println("In Selectrange with ni index************");
            for (Comparable[] record:tuples) {
                if((new KeyType(record[match(key)[0]]).compareTo(keyVal1)>=0)&&(new KeyType(record[match(key)[0]]).compareTo(keyVal2)<0) ){
                   rows.add(record);
    ...
    /************************************************************************************
	 * Join this table and table2 by performing an "equi-join". Same as above,
	 * but implemented using a Hash Join algorithm. *
     * @param attributes1
     *             the attributes of this table to be compared (Foreign Key)
	 * @param attributes2
	 *            the attributes of table2 to be compared (Primary Key)
    ...
    /************************************************************************************
     * Return a tuple at index n in table.
     *
     * @param n  index of tuple
     * @return  tuple
     */
    public Comparable [] getRow (int n)
    {
        Comparable[] res = new Comparable[domain.length];
        if (n < tuples.size())
            res = tuples.get(n);
        else
            System.out.println("Number of tuples in table: " + tuples.size());
        return res;
    } // getRow
